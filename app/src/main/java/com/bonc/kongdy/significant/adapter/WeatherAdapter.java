    ...
    class NowWeatherViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(Context context, Weather weather) {
            try {
                ...
                tempPm.setText(MainUtils.safeText("PM25： ", weather.aqi.city.pm25));
                tempQuality.setText(MainUtils.safeText("空气质量： ", weather.aqi.city.qlty));

                Glide.with(context).load(Setting.getInstance().getInt(weather.now.cond.txt,R.mipmap.type_one_sunny)).crossFade().into(weatherIcon);
        ...
        public void bind(Context context, Weather weather) {
            try {
                ...
                for (int i = 0; i < weather.dailyForecast.size(); i++) {
                    if (i > 1) {
                        try {
                            forecastDate[i].setText(MainUtils.dayForWeek(weather.dailyForecast.get(i).date));
                        } catch (Exception e) {
                            Logger.e(e.toString());
                        }
                    }

                    Glide.with(context).load(mSetting.getInt(weather.dailyForecast.get(i).cond.txtD, R.mipmap.type_none)).crossFade().into(forecastIcon[i]);
